...
function wireCommandBar() {
  ...
  if (impBtn) impBtn.addEventListener("click", () => {
    ...
    inp.addEventListener("change", async () => {
      ...
      App.state.upf = text;
      App.state.filename = f.name || "pasted.upf";
  ...
  if (quickBtn) quickBtn.addEventListener("click", () => {
    ...
    menu.innerHTML = `<div class="cmd-menu-label">Quick actions</div>
      <button class="cmd-menu-item" data-q="sample" type="button">Load sample UPF</button>
  ...
  if (setBtn) setBtn.addEventListener("click", () => {
    ...
    menu.innerHTML = `<div class="cmd-menu-label">Workspace</div>
      ...
      <div class="cmd-menu-item"><span class="mono">UPF-Insight — Power-Intent Intelligence for Digital Design</span></div>`;
...
async function wirePage(view) {
  ...
  if (view === "new_analysis") wireNewAnalysis(main);
           ...
           || view === "coverage" || view === "readiness" || view === "support" || view === "design") wireCrossLinks(main);
}
...
function wireTestDrive(main) {
  ...
  tr.addEventListener("click", async () => {
    ...
    if (which === "regression") { await runRegression(); return; }
  ...
  const doValidate = async () => {
    ...
    try {
      ...
      if (gval) gval.innerHTML = sectionTitle("Inline validation", `${findings.length} finding(s)`) +
        ...
        </tbody></table></div>` : '<p class="callout co-success"><span><strong>Clean</strong> — generated UPF passed.</span></p>');
    } catch (e) { setStatus(`<span class="gen-bad">${esc(e.message)}</span>`); }
  };
